/*!
@file
@brief Program using the Ball FMU in Model Exchange modality.
   ...
   fmi2Real                 states[] )
{
   ...
   fmi2Real work_deriv[NUM_MODEL_STATES];
   fmi2Status exec_loop_status = fmi2OK;

   /* Calculate time, states and derivatives at the beginning of the step. */
   exec_loop_status = fmu.fmi2SetTime( curr_time );
   ...
   for ( sinc = 0 ; sinc < num_states ; sinc++ ) {
      states[sinc] += integ_step * derivs[sinc];
   }

   /* Calculate the derivatives at the end of the Euler step. */
   exec_loop_status = fmu.fmi2SetTime( curr_time + integ_step );
   if ( exec_loop_status != fmi2OK ) {
      printf( "Error with fmi2SetTime!\n" );
      return( exec_loop_status );
   }
   ...
   for ( sinc = 0 ; sinc < num_states ; sinc++ ) {
      states[sinc] = work_state[sinc] + ((work_deriv[sinc] + derivs[sinc]) * dto2);
   }

   /* Set the states and recompute the derivatives at the completed step. */
   exec_loop_status = fmu.fmi2SetContinuousStates( states, num_states );
   if ( exec_loop_status != fmi2OK ) {
      printf( "Error with fmi2SetContinuousStates!\n" );
      return( exec_loop_status );
...
int main( int nargs, char ** args )
{
   ...
   stop_time  = 100.0;      // Set the ending time for the simulation.
   ...
   values[0] = 5.0;
   values[1] = 5.0;
   values[2] = 3.5 * cos( 45.0 * DTR );
   values[3] = 3.5 * sin( 45.0 * DTR );
   fmu.fmi2SetReal( vr, 4, values );
   ...
   fmu.fmi2GetReal( vr, num_var, values );
   outfile << setprecision( 15 ) << start_time;
   for ( iinc = 0 ; iinc < 6 ; iinc++ ) {
      outfile << ", " << setprecision( 15 ) << values[iinc];
   }
   outfile << ", " << setprecision( 15 ) << values[7];
   outfile << ", " << setprecision( 15 ) << values[8];
   outfile << std::endl;

   // 10. Simulate with the FMU
   frame_count = 0;
   frame_size  = 0.01;
